...
For development against the IDE, `./mvnw spring-boot:run` starts PostgreSQL on its own through
`compose-dev.yaml` (spring-boot-docker-compose) and leaves it up between restarts.
...
> DB_URL=jdbc:postgresql://localhost:5433/tenantbase_auth \
...
Then uncomment the volume and the two `TENANTBASE_SECURITY_JWT_*_KEY_LOCATION` variables in `compose.yaml`.
`keys/` is git-ignored. In production these belong in a secret manager, mounted read-only.
...
credentials and roles stay here.
